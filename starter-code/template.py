...
def compare_models(prompt: str) -> dict:
    ...
    return {
        "gpt4o": {
            ...
            "cost": _calculate_cost(OPENAI_MODEL, gpt4o_usage["input_tokens"], gpt4o_usage["output_tokens"]),
        ...
        "gpt4o_mini": {
            ...
            "cost": _calculate_cost(OPENAI_MINI_MODEL, gpt4o_mini_usage["input_tokens"], gpt4o_mini_usage["output_tokens"]),
        ...
        "gemini_flash": {
            ...
            "cost": _calculate_cost(GEMINI_MODEL, gemini_usage["input_tokens"], gemini_usage["output_tokens"]),
...
def streaming_chatbot() -> None:
    ...
    print("Gemini 2.5 Flash streaming chatbot. Gõ 'quit' hoặc 'exit' để thoát.")
    while True:
        prompt_text = input("Bạn: ").strip()
        if not prompt_text:
            continue
        ...
        print(f"(Đã nhận {len(assistant_text)} ký tự trong {latency:.2f}s)")
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        for model_key, label in model_labels:
            ...
            lines.append(
                f"| {prompt} | {label} | {truncated} | {stats.get('latency', 0):.2f}s | "
                f"{stats.get('input_tokens', 0)}/{stats.get('output_tokens', 0)} | ${stats.get('cost', 0):.8f} |"
            )
    return "\n".join(lines)
